...
<h1>Solteq: Employee Registration</h1>
...
<aui:form action="<%= createAccountURL %>" method="post" name="fm" validateOnBlur="<%= false %>">
    ...
    <c:if test="${errorMsg ne null}">
        <div class="portlet-msg-error my-3">${errorMsg}</div>
    </c:if>
			...
			<div class="form-group">
				...
				<clay:row>
					<clay:col
						md="6"
					>
                        <aui:input model="<%= User.class %>" name="emailAddress" required="true" />
                        ...
                        <aui:select name="jobTitle" label="employee.jobTitle" showEmptyOption="true">
                            <c:forEach var="jobTitle" items="<%= JobTitle.values() %>">
                                <aui:option value="${jobTitle.label}" label="${jobTitle.label}" />
                            </c:forEach>
                        </aui:select>

						<aui:input type="text" name="phoneNumber" required="true" />

					</clay:col>
                    <clay:col md="6">

                        <aui:select name="companyName" label="employee.companyName" showEmptyOption="true">
                            <c:forEach var="companyName" items="<%= CompanyName.values() %>">
                                <aui:option value="${companyName.key}" label="${companyName.label}" />
                            </c:forEach>
                        </aui:select>
                        <aui:select name="department" label="employee.department" showEmptyOption="true">
                            <c:forEach var="department" items="<%= Department.values() %>">
                                <aui:option value="${department.key}" label="${department.label}" />
                            </c:forEach>
                        </aui:select>
                        <aui:select name="workLocation" label="employee.workLocation" showEmptyOption="true">
